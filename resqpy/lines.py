...
def load_hdf5_array(object, node, array_attribute, tag = 'Values'):
   ...
   return object.model.h5_array_element(h5_key_pair, index = None, cache_array = True,
...
class Polyline(_BasePolyline):
    ...
    def is_convex(self, trust_metadata = True):
        ...
        if not self.isclosed: return False
        ...
        if nodes < 3:
            ...
            for node in range(3, nodes):
                ...
                if cw == 0.0: continue  # striaght line section
                if cw_bool is None:
                    cw_bool = (cw > 0.0)
                elif cw_bool != (cw > 0.0):
                    result = False
                    break
            if cw_bool is None: result = False   # whole polyline lies in a straight line
            if result: extras['is_clockwise'] = cw_bool
    ...
    def point_is_inside_xy(self, p, mode = 'crossing'):
        ...
        if mode == 'crossing': return pip.pip_cn(p, self.coordinates)
    ...
    def segment_length(self, segment_index, in_xy = False):
        ...
        if self.isclosed:
            ...
            if segment_index == len(self.coordinates) - 1: successor = 0
        else:
    ...
    def equidistant_points(self, n, in_xy = False):
        ...
        if self.isclosed: step_fraction = 1.0 / float(n)
        else: step_fraction = 1.0 / float(n - 1)
        f = 0.0
        for step in range(n):
            sample_points[step] = self.interpolated_point(f, in_xy = in_xy)
            f += step_fraction
            if f > 1.0: f = 1.0   # to avoid possible assertion error due to rounding
    ...
    def balanced_centre(self, mode = 'weighted', n = 20, cache = True, in_xy = False):
        ...
        if cache and self.centre is not None: return self.centre
    ...
    def first_line_intersection(self, x1, y1, x2, y2, half_segment = False):
        ...
        for segment in range(seg_count):
            ...
            x, y = meet.line_line_intersect(x1, y1, x2, y2,
                                            self.coordinates[segment][0], self.coordinates[segment][1],
                                            self.coordinates[successor][0], self.coordinates[successor][1],
                                            line_segment = True, half_segment = half_segment)
            if x is not None: return segment, x, y
    ...
    def normalised_xy(self, x, y, mode = 'square'):
        ...
        if tuple(centre_xy) == (x, y):
            if mode == 'square': return 0.5, 0.5
            return 0.0, 0.0
        segment, px, py = self.first_line_intersection(centre_xy[0], centre_xy[1], x, y, half_segment = True)
        assert px is not None
        norm_x, norm_y = None, None
        if mode == 'square':
            if px == centre_xy[0]: norm_x = 0.5
            else: norm_x = 0.5  +  0.5 * (x - centre_xy[0]) / abs(px - centre_xy[0])
            if py == centre_xy[1]: norm_y = 0.5
            else: norm_y = 0.5  +  0.5 * (y - centre_xy[1]) / abs(py - centre_xy[1])
        elif mode == 'circle':
            ...
            if abs(dp_xy[0]) > abs(dp_xy[1]):
                ...
                if d_xy[0] < 0.0: theta += maths.pi
            else:
                rf = d_xy[1] / dp_xy[1]
                theta = maths.pi / 2.0  -  maths.atan(d_xy[0] / d_xy[1])
                if d_xy[1] < 0.0: theta += maths.pi
            norm_x = rf * rf
            theta /= 2.0 * maths.pi
            if theta < 0.0: theta += 1.0
            elif theta > 1.0: theta -= 1.0
            norm_y = theta
        elif mode == 'perimeter':
            d_xy = np.array((x, y)) - centre_xy
            dp_xy = np.array((px, py)) - centre_xy
    ...
    def denormalised_xy(self, norm_x, norm_y, mode = 'perimeter'):
        ...
        x, y = None, None
        if mode == 'square':
            if (norm_x, norm_y) == (0.5, 0.5): return tuple(centre_xy)
            ...
            if px == centre_xy[0]: x = centre_xy[0]
            else: x = centre_xy[0]  +  f * (px - centre_xy[0])
            if py == centre_xy[1]: y = centre_xy[1]
            else: y = centre_xy[1]  +  f * (py - centre_xy[1])
    ...
    def create_xml(self, ext_uuid = None, add_as_part = True, add_relationships = True, root = None,
        ...
        if self.rep_int_root is not None:
            ...
            self.model.create_ref_node('RepresentedInterpretation',
                                       ...
                                       rep_int.attrib['uuid'],
                                       content_type = content_type, root = polyline)
        ...
        if root is not None: root.append(polyline)
        if add_as_part:
            ...
            if add_relationships:
                ...
                if self.rep_int_root is not None: # Optional
                    self.model.create_reciprocal_relationship(polyline, 'destinationObject', self.rep_int_root, 'sourceObject')
...
class PolylineSet(_BasePolyline):
    ...
    def __init__(self, parent_model, set_root = None, uuid = None, polylines = None, irap_file = None, charisma_file = None,
        ...
        if polylines is not None: # Create from list of polylines
            ...
            for crs_uuid in crs_set:
                ...
                if self.crs_root is not None: break
            ...
            for i, line in enumerate(inpoints):
                ...
                if i == 0:
                    ...
                    self.coordinates = np.concatenate((self.coordinates,np.array(([[float(line[3]),float(line[4]),float(line[5])]]))))
                    count +=1
    ...
    def poly_index_containing_point_in_xy(self, p, mode = 'crossing'):
        ...
        for i, poly in enumerate(self.polys):
            if not poly.isclosed: continue
            if poly.point_is_inside_xy(p, mode = mode): return i
        return None
    ...
    def create_xml(self, ext_uuid = None, add_as_part = True, add_relationships = True, root = None, title = None,
        ...
        if self.rep_int_root is not None:
            ...
            self.model.create_ref_node('RepresentedInterpretation',
                                       rqet.citation_title_for_node(rep_int),
                                       rep_int.attrib['uuid'],
                                       content_type = content_type, root = polyset)
        ...
        if root is not None: root.append(polyset)
        if add_as_part:
            ...
            if add_relationships:
                ...
                if self.rep_int_root is not None: # Optional
                    self.model.create_reciprocal_relationship(polyset, 'destinationObject', self.rep_int_root, 'sourceObject')
                if self.save_polys:
                    for poly in self.polys:
                        self.model.create_reciprocal_relationship(polyset, 'destinationObject', poly.root_node, 'sourceObject')
...
def spline(points, tangent_vectors = None, tangent_weight = 'square',
    ...
    if closed: seg_lengths[-1] = vu.naive_length(points[0] - points[-1])
    ...
    if max_segment_length is not None:
        for seg in range(seg_count):
            ...
            if sub_count > knot_insertions[seg] + 1: knot_insertions[seg] = sub_count - 1
