...
class GridFromCp:
    ...
    def __get_dot_mask_dots(self):
        ...
        self.__dot_mask = np.all(np.abs(self.__cp_array[:, :, :, 1, 1, 1] - self.__cp_array[:, :, :, 0, 0, 0]) < self.__dot_tolerance, axis=-1)
    ...
    def __get_dot_mask_ijdots_or_morse(self):
        ...
        self.__dot_mask[:] = np.where(np.all(np.abs(j_face_vecs[:, :, :, 0]) < self.__dot_tolerance, axis=-1), True, self.__dot_mask)
        self.__dot_mask[:] = np.where(np.all(np.abs(j_face_vecs[:, :, :, 1]) < self.__dot_tolerance, axis=-1), True, self.__dot_mask)
        self.__dot_mask[:] = np.where(np.all(np.abs(i_face_vecs[:, :, :, 0]) < self.__dot_tolerance, axis=-1), True, self.__dot_mask)
        self.__dot_mask[:] = np.where(np.all(np.abs(i_face_vecs[:, :, :, 1]) < self.__dot_tolerance, axis=-1), True, self.__dot_mask)
    ...
    def __get_dot_mask_morse(self, i_face_vecs, j_face_vecs):
        ...
        self.__dot_mask = np.where(np.any(len_j_face_vecs_sqr > morse_tol_sqr * max_len_active_j_face_vecs_sqr, axis=-1), True,
                            self.__dot_mask)
        self.__dot_mask = np.where(np.any(len_i_face_vecs_sqr > morse_tol_sqr * max_len_active_i_face_vecs_sqr, axis=-1), True,
                            self.__dot_mask)
        ...
        if self.__all_active and self.__geometry_defined_everywhere:
            ...
            if not self.__geometry_defined_everywhere and not self.__all_active:
                if self.__treat_as_nan == 'inactive':
                    ...
                    self.__cp_nan_mask = np.logical_or(self.__cp_nan_mask, np.logical_and(self.__inactive_mask, self.__dot_mask))
    ...
    def __get_masked_cp_array(self):
        ...
        if self.__geometry_defined_everywhere:
            ...
            full_mask = self.__cp_nan_mask.reshape((self.__nk, self.__nj, self.__ni, 1)).repeat(24, axis=3).reshape((self.__nk, self.__nj, self.__ni, 2, 2, 2, 3))
    ...
    def __get_k_reduced_cp_array(self):
        ...
        self.__k_reduced_cp_array = ma.masked_array(
            np.zeros((self.__nk_plus_1, self.__nj, self.__ni, 2, 2, 3)))  # (nk+1+k_gaps, nj, ni, jp, ip, xyz)
        ...
        if self.__k_gaps:
            ...
            self.__k_reduced_cp_array[1:-1, :, :, :, :, :] = np.where(slice.mask, self.__masked_cp_array[:-1, :, :, 1, :, :, :], slice)
    ...
    def __get_k_reduced_cp_array_kgaps(self):
        ...
        for k in range(self.__nk - 1):
            ...
            if self.__k_gap_after_layer[k]:
                ...
                self.__k_reduced_cp_array[raw_k, :, :, :, :, :] = np.where(slice.mask, self.__masked_cp_array[k, :, :, 1, :, :, :],
                                                                    slice)
                raw_k += 1
    ...
    def __get_primary_pillar_ref(self):
        ...
        self.__primary_pillar_jip = np.zeros((self.__nj_plus_1, self.__ni_plus_1, 2), dtype='int')  # (nj + 1, ni + 1, jp:ip)
        ...
        for j in range(self.__nj_plus_1):
            for i in range(self.__ni_plus_1):
                if self.__active_mask_2D[j - self.__primary_pillar_jip[j, i, 0], i - self.__primary_pillar_jip[j, i, 1]]:
                    continue
                ...
                if j > 0 and self.__primary_pillar_jip[j, i, 0] == 0 and self.__active_mask_2D[
                    ...
                    continue
                if i > 0 and j > 0 and self.__primary_pillar_jip[j, i,
                                                          0] == 0 and self.__primary_pillar_jip[j, i,
                                                                                         1] == 0 and self.__active_mask_2D[
                    j - 1,
                    i - 1]:
                    self.__primary_pillar_jip[j, i, :] = 1

    def __get_extra_pillar_ref(self):
        self.__extras_count = np.zeros((self.__nj_plus_1, self.__ni_plus_1), dtype='int')  # count (0 to 3) of extras for pillar
        self.__extras_list_index = np.zeros((self.__nj_plus_1, self.__ni_plus_1), dtype='int')  # index in list of 1st extra for pillar
        self.__extras_list = []  # list of (jp, ip)
        self.__extras_use = np.negative(np.ones((self.__nj, self.__ni, 2, 2), dtype='int'))  # (j, i, jp, ip); -1 means use primary
        ...
        if self.__split_pillars:
            ...
            for j in range(self.__nj_plus_1):
                for i in range(self.__ni_plus_1):
                    ...
                    self.__points_array[:, j, i, :] = np.where(slice.mask, np.nan, slice)  # NaN indicates undefined/invalid geometry

    def __get_points_array_split(self):
        self.__points_array = np.zeros(
            (self.__nk_plus_1, (self.__nj_plus_1 * self.__ni_plus_1) + len(self.__extras_list), 3))  # note: nk_plus_1 might include k_gaps
        index = 0
    ...
    def __update_grid_geometry_information(self):
        ...
        if self.__geometry_defined_everywhere:
            ...
            self.grid.array_cell_geometry_is_defined = self.__active_mask.copy()  # a bit harsh: disallows reactivation of cells
    ...
    def __update_grid_handedness(self):
        ...
        if self.__ijk_handedness is None or self.__ijk_handedness == 'auto':
            ...
            self.__cell_ijk_lefthanded = (vec.clockwise(sample_cp[0, 0, 0], sample_cp[0, 1, 0], sample_cp[0, 0, 1]) >= 0.0)
