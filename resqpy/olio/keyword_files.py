...
def end_of_file(ascii_file):
   ...
   if len(line) == 0: return True    # end of file
...
def skip_blank_lines_and_comments(ascii_file, comment_char = '!', skip_c_space = True):
   ...
   while True:
      file_pos = ascii_file.tell()
      line = ascii_file.readline()
      if len(line) == 0: return         # end of file
      words = line.split()
      if len(words) == 0: continue
      if words[0][0] == comment_char: continue
...
def find_keyword_without_passing(ascii_file, keyword, no_pass_keyword):
   ...
   while True:
      file_pos = ascii_file.tell()
      line = ascii_file.readline()
      if len(line) == 0: return False    # end of file
      words = line.split()
      if len(words) > 0:
...
def find_keyword_pair(ascii_file, primary_keyword, secondary_keyword):
   ...
   while True:
      if not find_keyword(ascii_file, primary_keyword): return False
      file_pos = ascii_file.tell()
      line = ascii_file.readline()
      if len(line) == 0: return False    # end of file
      words = line.split()
...
def specific_keyword_next(ascii_file, keyword, skip_blank_lines = True, comment_char = '!'):
   ...
   while True:
      if (skip_blank_lines): file_pos = ascii_file.tell()   # will advance file pos to next non-blank line
      line = ascii_file.readline()
      if len(line) == 0:             # end of file
         ascii_file.seek(file_pos)
         return False
      words = line.split()
      if len(words) == 0 or words[0][0] == comment_char: continue    # blank line or comment
