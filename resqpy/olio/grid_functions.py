...
def infill_block_geometry(extent, depth, thickness, x, y,
   ...
   if k_increase_direction == 'down': k_dir_sign = 1.0
   elif k_increase_direction == 'up': k_dir_sign = -1.0
   else: assert(False)
   ...
   for j in range(extent[1]):
      for i in range(extent[2]):
         ...
         while k_top < extent[0] and abs(depth[k_top, j, i]) <= depth_zero_tolerance:
            ...
            if abs(x[k_top, j, i]) <= x_y_zero_tolerance: x[k_top, j, i] = 0.0
            if abs(y[k_top, j, i]) <= x_y_zero_tolerance: y[k_top, j, i] = 0.0
            k_top += 1                                                             # skip topmost inactive batch
         if k_top >= extent[0]: continue                                           # whole column is inactive
         ...
         while True:
            ...
            while k_base < extent[0] and abs(depth[k_base, j, i]) <= depth_zero_tolerance:
               ...
               if abs(x[k_base, j, i]) <= x_y_zero_tolerance: x[k_base, j, i] = 0.0
               if abs(y[k_base, j, i]) <= x_y_zero_tolerance: y[k_base, j, i] = 0.0
            ...
            if void_interval < 0.0:   # overlapping cells
               if -void_interval < vertical_cell_overlap_tolerance:
                  ...
                  log.warn('Cells [%1d, %1d, %1d] and [%1d, %1d, %1d] overlap ...', i+1, j+1, k_top, i+1, j+1, k_base+1)
...
def random_cell(corner_points, border = 0.25, max_tries = 20, tolerance = 0.003):
   ...
   for axis in range(3):
      ...
      if kji_upper[axis] < kji_border[axis]: kji_upper[axis] = kji_border[axis]
   ...
   while attempt < max_tries:
      ...
      if vec.manhatten_distance(cell_cp[0, 0, 0], cell_cp[1, 0, 0]) < tolerance: continue
      if vec.manhatten_distance(cell_cp[0, 0, 0], cell_cp[0, 1, 0]) < tolerance: continue
      if vec.manhatten_distance(cell_cp[0, 0, 0], cell_cp[0, 0, 1]) < tolerance: continue
...
def determine_corp_ijk_handedness(corner_points, xyz_is_left_handed = True):
   ...
   det = vec.determinant(cell_cp[0, 0, 1] - origin, cell_cp[0, 1, 0] - origin, cell_cp[1, 0, 0] - origin) # NB. IJK ordering
   if det == 0.0:
   ...
   if ijk_is_left_handed: return 'left'
   return 'right'
...
def determine_corp_extent(corner_points, tolerance = 0.003):
   ...
   def neighbours(corner_points, sextuple_cell_a_p1, sextuple_cell_a_p2,
                                 sextuple_cell_b_p1, sextuple_cell_b_p2, tolerance):
      # allows for reversal of points (or not) in neighbouring cell
      if ((vec.manhatten_distance(corner_points[sextuple_cell_a_p1], corner_points[sextuple_cell_b_p1]) <= tolerance) and
          (vec.manhatten_distance(corner_points[sextuple_cell_a_p2], corner_points[sextuple_cell_b_p2]) <= tolerance)):
         return True
      if ((vec.manhatten_distance(corner_points[sextuple_cell_a_p1], corner_points[sextuple_cell_b_p2]) <= tolerance) and
   ...
   while redundancy:
      ...
      for e in possible_extents:
         ...
         if candidate >= cell_count: continue
         if neighbours(corner_points,
                       (0, 0, kji_cell[2], 0, 1, 0),
                       (0, 0, kji_cell[2], 0, 1, 1),
                       (0, 0, candidate, 0, 0, 0),
                       (0, 0, candidate, 0, 0, 1), tolerance):
   ...
   while redundancy:
      ...
      for e in possible_extents:
         ...
         if candidate >= cell_count: continue
         if vec.manhatten_distance(corner_points[0, 0, kji_cell[2], 1, 0, 0],
                                   corner_points[0, 0, candidate,   0, 0, 0]) <= tolerance:
      ...
      if not found:
         remaining_attempts -= 1
         if remaining_attempts <= 0:
            log.error('failed to determine NJ from corner points (out of tries)')  # could assume or check if NK = 1 here
            return None
...
def translate_corp(corner_points, x_shift = None, y_shift = None, min_xy = None, preserve_digits = None):
   ...
   assert(corner_points.ndim == 7)
   if min_xy is None: minimum_xy = 0.0
   else: minimum_xy = min_xy
   if x_shift is None:
...
def triangles_for_cell_faces(cp):
   ...
   tri[0, :, :, 0] = np.mean(cp, axis = (1, 2)).reshape((2, 1, 3)).repeat(4, axis = 1).reshape((2, 4, 3))  # k face centres
   tri[1, :, :, 0] = np.mean(cp, axis = (0, 2)).reshape((2, 1, 3)).repeat(4, axis = 1).reshape((2, 4, 3))  # j face centres
   tri[2, :, :, 0] = np.mean(cp, axis = (0, 1)).reshape((2, 1, 3)).repeat(4, axis = 1).reshape((2, 4, 3))  # i face centres
...
def actual_pillar_shape(pillar_points, tolerance = 0.001):
   ...
   from_top = pp - pp[0]
   xy_drift = np.abs(from_top[:, :, 0]) + np.abs(from_top[:, :, 1])  # use Manhattan distance as cheap proxy for true distance
   if np.max(xy_drift) <= tolerance: return 'vertical'
   if np.max(xy_drift[-1]) <= tolerance: return 'curved'  # top & bottom are vertically aligned, so pillar must be curved
   ...
   xy_drift = from_top[:, :, :2]  -  z_fraction.reshape((pp.shape[0], pp.shape[1], 1)) * from_top[-1, :, :2].reshape((1, pp.shape[1], 2))
   straight = (np.max(np.sum(np.abs(xy_drift), axis = -1), axis = 0) <= tolerance)
   masked_straight = np.where(null_pillar_mask, True, straight)
   if np.all(masked_straight): return 'straight'
   return 'curved'
...
def columns_to_nearest_split_face(grid):
   ...
   if not grid.has_split_coordinate_lines: return None
   ...
   while True:
      ...
      updated[:] = np.minimum(updated, plus_one[1:-1, 2:])
      if np.all(updated == framed[1:-1, 1:-1]): break
...
def left_right_foursome(full_pillar_list, p_index):
   ...
   if entry == (0, 1):
      if exit == (-1, 0): return np.array([[False, True], [True, True]], dtype = bool)
      elif exit == (0, 1): return np.array([[False, False], [True, True]], dtype = bool)
      elif exit == (1, 0): return np.array([[False, False], [True, False]], dtype = bool)
      else:
         raise Exception('code failure whilst taking exit sides from dubious full pillar list')
   elif entry == (0, -1):
      if exit == (-1, 0): return np.array([[False, True], [False, False]], dtype = bool)
      elif exit == (0, -1): return np.array([[True, True], [False, False]], dtype = bool)
      elif exit == (1, 0): return np.array([[True, True], [True, False]], dtype = bool)
      else:
         raise Exception('code failure whilst taking exit sides from dubious full pillar list')
   elif entry == (1, 0):
      if exit == (0, -1): return np.array([[True, False], [False, False]], dtype = bool)
      elif exit == (1, 0): return np.array([[True, False], [True, False]], dtype = bool)
      elif exit == (0, 1): return np.array([[True, False], [True, True]], dtype = bool)
      else:
         raise Exception('code failure whilst taking exit sides from dubious full pillar list')
   elif entry == (-1, 0):
      if exit == (0, -1): return np.array([[True, True], [False, True]], dtype = bool)
      elif exit == (-1, 0): return np.array([[False, True], [False, True]], dtype = bool)
      elif exit == (0, 1): return np.array([[False, False], [False, True]], dtype = bool)
      else:
         raise Exception('code failure whilst taking exit sides from dubious full pillar list')
   else:
