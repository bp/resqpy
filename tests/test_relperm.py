...
class TestRelPerm(unittest.TestCase):
   ...
   def test_col_headers(self):
      ...
      self.assertTrue(
         "incorrect saturation column name and/or multiple saturation columns exist" in str(excval1.exception))
      ...
      self.assertTrue("capillary pressure data should be in the last column of the dataframe" in str(excval3.exception))
   ...
   def test_relperm(self):
      ...
      dataframe1 = RelPerm(model = self.model,
                           df = df1,
                           uom_list = uom_list,
                           phase_combo = phase_combo1,
                           low_sal = low_sal,
                           title = 'table1')
      dataframe2 = RelPerm(model = self.model,
                           df = df2,
                           uom_list = uom_list,
