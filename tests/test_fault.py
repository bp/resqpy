...
def test_add_connection_set_and_tmults(example_model_with_properties, test_data_path, inc_list, tmult_dict,
    ...
    for fault in faults:
        ...
        assert metadata["Transmissibility multiplier"] == str(float(expected_mult[
                                                                        title])), f'Expected mult for fault {title} to be {float(expected_mult[title])}, found {metadata["Transmissibility multiplier"]}'
