...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                ...
                dict(feature_name='foobar', kind='geobody boundary'),  # TODO: Allow age to be passed into init so coverage can be increased
        ),
        (
                rqo.WellboreFeature,
                dict(feature_name='foobar'),
...
def test_GeobodyInterp(tmp_model):
    ...
    gb = rqo.GeobodyInterpretation(tmp_model, geobody_feature=gi, domain='depth', composition='intrusive clay', material_implacement='autochtonous', geobody_shape='silt')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                rqo.BoundaryFeatureInterpretation(parent_model="test", boundary_feature=rqo.BoundaryFeature("test_2"),
                                                  title='title')
        )
    ])
...
@pytest.mark.parametrize(
    "fi_1_throw_list, fi_2_throw_list, expected_output",
    [
        (
                ['string', 'left', 'right'],
                ['string', 'left', 'right'],
...
def test_GeobodyBoundaryInterpretation_is_equivalent_false_3(tmp_model):
    ...
    gbi_2 = rqo.GeobodyBoundaryInterpretation(tmp_model, genetic_boundary_feature = gbf, domain='depth', extra_metadata={'metadata': 'metadata'})
...
def test_GeobodyBoundaryInterpretation_is_equivalent_false_4(tmp_model):
    # Arrange
    gbf = rqo.GeneticBoundaryFeature(tmp_model)
    gbi_1 = rqo.GeobodyBoundaryInterpretation(tmp_model, genetic_boundary_feature = gbf, domain='depth', extra_metadata={'metadata': 'metadata'})
    gbi_2 = rqo.GeobodyBoundaryInterpretation(tmp_model, genetic_boundary_feature = gbf, domain='test', extra_metadata={'metadata': 'metadata'})
...
@pytest.mark.parametrize(
    "relation_list_1, relation_list_2, expected_result",
    [
        (
                None,
                None,
...
def test_GeobodyBoundaryInterpretation_is_equivalent_5(tmp_model, relation_list_1, relation_list_2, expected_result):
    # Arrange
    gbf = rqo.GeneticBoundaryFeature(tmp_model)
    gbi_1 = rqo.GeobodyBoundaryInterpretation(tmp_model, genetic_boundary_feature = gbf, domain='depth', extra_metadata={'metadata': 'metadata'}, boundary_relation_list=relation_list_1)
    gbi_2 = rqo.GeobodyBoundaryInterpretation(tmp_model, genetic_boundary_feature = gbf, domain='depth', extra_metadata={'metadata': 'metadata'}, boundary_relation_list=relation_list_2)
...
@pytest.mark.parametrize(
    "extra_metadata_1, extra_metadata_2, domain_1, domain_2, expected_result",
    [
        (
                {'metadata': 'metadata'},
                {'metadata': 'test'},
...
def test_GeobodyInterpretation_is_equivalent_false_2(tmp_model, extra_metadata_1, extra_metadata_2, domain_1, domain_2, expected_result):
    # Arrange
    gf = rqo.GeobodyFeature(tmp_model)
    gi_1 = rqo.GeobodyInterpretation(tmp_model, geobody_feature=gf, extra_metadata=extra_metadata_1, domain=domain_1, )
    gi_2 = rqo.GeobodyInterpretation(tmp_model, geobody_feature=gf, extra_metadata=extra_metadata_2, domain=domain_2)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                rqo.GeologicUnitFeature('model', feature_name='name', extra_metadata={'metadata': 'metadata'},)
        ),
    ])
def test_GeologicUnitFeature_is_equivalent_false(tmp_model, guf_2):
    # Arrange
    guf_1 = rqo.GeologicUnitFeature(tmp_model, feature_name='test', extra_metadata={'metadata': 'metadata'},)
...
def test_HorizonInterpretation_is_equivalent_false(tmp_model, hi_2):
    # Arrange
    hi_1 = rqo.HorizonInterpretation(tmp_model, extra_metadata={'metadata': 'test'},)
...
def test_HorizonInterpretation_is_equivalent_false_2(tmp_model):
    # Arrange
    hi_1 = rqo.HorizonInterpretation(tmp_model, extra_metadata={'metadata': 'metadata'},)
    hi_2 = rqo.HorizonInterpretation(tmp_model, extra_metadata={'metadata': 'test'},)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                rqo.RockFluidUnitFeature(parent_model="test")
        )
    ])
