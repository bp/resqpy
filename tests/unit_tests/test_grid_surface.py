...
def test_find_faces_to_represent_surface_regular_optimised(small_grid_and_surface):
    ...
    gcs_optimised = rqgs.find_faces_to_represent_surface_regular_optimised(
        grid, surface, name
    )
...
def test_find_faces_to_represent_surface_regular_optimised_with_return_properties(
    small_grid_and_surface,
):
    # Arrange
    grid = small_grid_and_surface[0]
    surface = small_grid_and_surface[1]
    ...
    # Act
    gcs_normal, properties_dict = rqgs.find_faces_to_represent_surface_regular(
    ...
    ) = rqgs.find_faces_to_represent_surface_regular_optimised(
        grid, surface, name, return_properties=return_properties
    )
    cip_optimised = gcs_optimised.cell_index_pairs
    fip_optimised = gcs_optimised.face_index_pairs
    ...
    np.testing.assert_array_almost_equal(
        normal_vectors_normal, normal_vectors_optimised
    )
